...
class SupersetAPI:
    ...
    def create_chart(self, name, chart_type, groupby=[], metrics=[], query=None):
        ...
        if metrics:
            for m in metrics:
                formatted_metrics.append({
                    ...
                    "aggregate": m.upper() if isinstance(m, str) else m.get("aggregate", "COUNT").upper(),
                    "label": m if isinstance(m, str) else m.get("label", "count")
                })
        else:
            formatted_metrics = [{
                "expressionType": "SIMPLE",
        ...
        if chart_type in ["line", "area", "time_series", "time-series", "time-series bar"] and self.datetime_column:
            params.update({
        ...
        if chart_type == "pie":
            params.update({
                "metrics": formatted_metrics[0],  # pie expects single metric
